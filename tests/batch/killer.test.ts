...
const COUNTS: Record<Level, number> = FULL ? { 1: 200, 2: 200, 3: 200, 4: 50, 5: 20 } : { 1: 6, 2: 6, 3: 6, 4: 4, 5: 3 };

describe('杀手数独批量生成验收', () => {
  const rows: ReturnType<typeof runBatch>[] = [];
  for (const level of [1, 2, 3, 4, 5] as Level[]) {
    it(`第 ${level} 档 × ${COUNTS[level]}`, () => {
      const st = runBatch('killer', level, COUNTS[level], 31337 + level * 1_000_003);
      rows.push(st);
      expect(st.failures).toBe(0);
    }, 3_600_000);
  }
  it('各档步数与难度分随档位单调上升', () => {
    writeReport('killer', rows);
